...
All spatial queries are resolved at initialisation time to eliminate per-node grid operations during the search:

| Table | Content | Lookup |
| --- | --- | --- |
| `valid_pos[i][idx]` | Can block *i* physically fit at index *idx*? | O(1) |
| `block_masks[i][idx]` | Bitmask of cells occupied by block *i* at *idx* | O(1) |
| `gate_exit[i][idx]` | Is block *i* at *idx* aligned with a matching gate? | O(1) |
| `slide_rays[i][idx]` | Ordered `(new_idx, step_mask)` per direction | O(ray length) |

### 4.2 Bitmask Collision in One Instruction

Board occupancy is maintained as a single integer bitmask (one bit per cell). Collision detection boils down to:

```
collision = (other_mask & step_mask) ≠ 0
```

On a 12×12 board this easily fits in a Python integer without needing NumPy or external libraries. The XOR trick `other_mask = board_mask ⊕ self_mask` cleanly strips the moving block's own footprint before testing.

### 4.3 Complexity Summary

| Operation | Complexity |
| --- | --- |
| State construction | O(B) where B = number of blocks |
| Single slide-step collision | O(1) bitwise AND |
| Successor generation | O(B × R) where R = avg ray length |
| Canonical state key | O(B log B) dominated by position sorting |
| Gate exit check | O(1) table lookup |
...
| Property | Complete A\* | Fast (Greedy) | Fringe |
| --- | --- | --- | --- |
| Queue | Min-heap (f = g + h) | Min-heap (f = h) | Dual stacks |
| Heuristic | Admissible (ice = 0) | Aggressive (ice = 3) | Aggressive (ice = 3) |
| Optimality | Guaranteed | None | Near-optimal |
| Duplicates | best\_g map (reopening) | Visited set (no reopen) | best\_g map (reopening) |
| Best for | Correctness-critical | Time-critical | Memory-bounded |
...
| Level | Moves | Time | Moves | Time | Moves | Time |
| --- | --- | --- | --- | --- | --- | --- |
| | **Complete** | **Complete** | **Fast** | **Fast** | **Fringe** | **Fringe** |
...
| test4.txt | 49 | 37.212 s | 72 | 18.554 s | 55 | 43.468 s |
